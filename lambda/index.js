...
async function play(h){const m=await meta(h);const intro=isIT(h)?'Radio Freccia Nera Classica.':'Radio Freccia Nera Classical.';return h.responseBuilder.speak(intro).addDirective({type:'AudioPlayer.Play',playBehavior:'REPLACE_ALL',audioItem:{stream:{url:STREAM,token:'rfn-classical-'+Date.now(),offsetInMilliseconds:0},metadata:{title:m.title,subtitle:m.subtitle,art:{sources:[{url:m.art}]},backgroundImage:{sources:[{url:m.art}]}}}}).withShouldEndSession(true).getResponse();}
