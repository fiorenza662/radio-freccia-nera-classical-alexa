...
const NOW='https://radio.radiofreccianera.com/api/nowplaying/classical';
const LOGO='https://www.radiofreccianera.com/wp-content/themes/radio-freccia-nera-thematic-network-en-v2.2/assets/img/cards-pro/classical.png';
...
async function meta(h){const d=await getNow();const s=d&&d.now_playing&&d.now_playing.song;const fallbackTitle=isIT(h)?'Radio Freccia Nera Classica':'Radio Freccia Nera Classical';if(!s)return{title:fallbackTitle,subtitle:'Radio Freccia Nera',art:LOGO};const artist=(s.artist||'').trim();const title=(s.title||'').trim();const text=(s.text||'').trim();const art=(s.art||'').trim()||LOGO;return{title:text||title||fallbackTitle,subtitle:artist||'Radio Freccia Nera',art};}
function streamDirective(tokenPrefix,m){m=m||{title:'Radio Freccia Nera Classical',subtitle:'Radio Freccia Nera',art:LOGO};return{type:'AudioPlayer.Play',playBehavior:'REPLACE_ALL',audioItem:{stream:{url:STREAM,token:tokenPrefix+'-'+Date.now(),offsetInMilliseconds:0},metadata:{title:m.title,subtitle:m.subtitle,art:{sources:[{url:m.art}]},backgroundImage:{sources:[{url:m.art}]}}}};}
async function play(h){const m=await meta(h);const intro=isIT(h)?'Radio Freccia Nera Classica.':'Radio Freccia Nera Classical.';return h.responseBuilder.speak(intro).addDirective(streamDirective('rfn-classical',m)).withShouldEndSession(true).getResponse();}
...
const Recover={canHandle:h=>['AudioPlayer.PlaybackFailed','AudioPlayer.PlaybackFinished'].includes(Alexa.getRequestType(h.requestEnvelope)),handle:h=>h.responseBuilder.addDirective(streamDirective('rfn-classical-recover')).getResponse()};
